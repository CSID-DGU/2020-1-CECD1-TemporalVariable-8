...
func (cq *CategoryQuery) QueryOwner() *RestaurantQuery {
	query := &RestaurantQuery{config: cq.config}
...
func (cq *CategoryQuery) QueryMenus() *MenuQuery {
	query := &MenuQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) WithOwner(opts ...func(*RestaurantQuery)) *CategoryQuery {
	query := &RestaurantQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].category_owner; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_owner" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withMenus; query != nil {
		...
		query.Where(predicate.Menu(func(s *sql.Selector) {
			s.Where(sql.InValues(category.MenusColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.category_menus
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_menus" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_menus" returned %v for node %v`, *fk, n.ID)
			}
