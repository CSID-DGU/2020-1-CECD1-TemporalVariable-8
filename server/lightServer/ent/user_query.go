...
func (uq *UserQuery) QueryAuths() *AuthorizeQuery {
	query := &AuthorizeQuery{config: uq.config}
...
//  WithAuths tells the query-builder to eager-loads the nodes that are connected to
// the "auths" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithAuths(opts ...func(*AuthorizeQuery)) *UserQuery {
	query := &AuthorizeQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAuths; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.Authorize(func(s *sql.Selector) {
		...
		for _, n := range neighbors {
			fk := n.user_auths
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_auths" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_auths" returned %v for node %v`, *fk, n.ID)
			}
