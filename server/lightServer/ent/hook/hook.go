...
// The OrderFieldFunc type is an adapter to allow the use of ordinary
// function as OrderField mutator.
type OrderFieldFunc func(context.Context, *ent.OrderFieldMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OrderFieldFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.OrderFieldMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OrderFieldMutation", m)
	}
	return f(ctx, mv)
}
